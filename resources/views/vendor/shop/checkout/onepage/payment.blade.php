            ...
            <template v-else>
                ...
                <x-shop::accordion class="overflow-hidden !border-b-0 max-md:rounded-lg max-md:!border-none max-md:!bg-gray-100">
                    ...
                    <x-slot:content class="mt-8 !p-0 max-md:mt-0 max-md:rounded-t-none max-md:border max-md:border-t-0 max-md:!p-4">
                        ...
                        <!-- QR Payment Panel — shown when a QR method is selected -->
                        <template v-if="selectedPayment && isQrMethod(selectedPayment.method) && selectedPayment.image">
                            <div class="mt-6 rounded-xl border border-blue-100 bg-blue-50 p-5">
                                <p class="mb-1 text-sm font-semibold text-gray-800">
                                    💳 掃描 QR code 完成付款
                                </p>
                                <p class="mb-4 text-xs text-gray-500">
                                    請掃描以下 QR code，輸入金額後完成付款，然後點擊「確認訂單」。
                                </p>
                                <div class="flex flex-col items-center gap-3">
                                    <img
                                        :src="selectedPayment.image"
                                        :alt="selectedPayment.method_title + ' QR Code'"
                                        class="h-52 w-52 rounded-xl object-contain shadow-md"
                                    >
                                    <p class="text-sm font-medium text-gray-700">
                                        @{{ selectedPayment.method_title }}
                                    </p>
                                    <p v-if="selectedPayment.description" class="text-center text-xs text-gray-500">
        ...
        app.component('v-payment-methods', {
            ...
            methods: {
                store(selectedMethod) {
                    this.selectedPayment = selectedMethod;
                ...
                isQrMethod(method) {
                    return ['payme', 'fps', 'paypalqr'].includes(method);
                },
            },
        });
    </script>
